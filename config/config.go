...
func (c *Config) Validate() error {
	...
	// 设置仓位大小默认值
	if c.PositionSize.MaxMarginUsagePct <= 0 {
		c.PositionSize.MaxMarginUsagePct = 80.0 // 默认80%（留20%缓冲）
	}
	if c.PositionSize.MaxMarginUsagePct > 95 {
		fmt.Printf("⚠️  警告: 最大保证金使用率设置为%.1f%%，风险较高，建议≤80%%\n", c.PositionSize.MaxMarginUsagePct)
	}
