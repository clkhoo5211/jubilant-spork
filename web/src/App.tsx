...
function App() {
  ...
  // 动态生成subtitle：基于trader的model_name
  const getSubtitle = () => {
    if (!traders || traders.length === 0) {
      return language === 'en' ? 'Loading...' : '加载中...';
    }
    const models = [...new Set(traders.map(t => t.ai_model.toUpperCase()))];
    if (models.length === 2) {
    ...
    return language === 'en' ? 'AI Trading Competition' : 'AI交易竞赛';
  };
            ...
            <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
              ...
              <div>
                ...
                <p className="text-xs mono hidden sm:block" style={{ color: '#848E9C' }}>
                  {getSubtitle()} · {language === 'en' ? 'Real-time' : '实时'}
                </p>
              </div>
            </div>
