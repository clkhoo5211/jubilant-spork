...
export function ComparisonChart({ traders }: ComparisonChartProps) {
  ...
  const combinedData = useMemo(() => {
    ...
    traderHistories.forEach((history, index) => {
      ...
      history.data.forEach((point: any) => {
        ...
        if (!timestampMap.has(ts)) {
          const time = new Date(ts).toLocaleTimeString(language === 'zh' ? 'zh-CN' : 'en-US', {
            hour: '2-digit',
            minute: '2-digit',
          });
          timestampMap.set(ts, {
    ...
    return combined;
  }, [allTraderHistories, traders, language]);

  if (isLoading) {
    return (
      <div className="text-center py-16" style={{ color: '#848E9C' }}>
        <div className="spinner mx-auto mb-4"></div>
        <div className="text-sm font-semibold">{t('loadingComparisonData', language)}</div>
      </div>
    );
  }
  ...
  if (combinedData.length === 0) {
    return (
      <div className="text-center py-16" style={{ color: '#848E9C' }}>
        ...
        <div className="text-lg font-semibold mb-2">{t('noComparisonData', language)}</div>
        <div className="text-sm">{t('comparisonDataWillAppear', language)}</div>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="mt-6 grid grid-cols-4 gap-4 pt-5" style={{ borderTop: '1px solid #2B3139' }}>
        ...
        <div className="p-3 rounded transition-all hover:bg-opacity-50" style={{ background: 'rgba(240, 185, 11, 0.05)' }}>
          ...
          <div className="text-base font-bold mono" style={{ color: '#EAECEF' }}>{combinedData.length} {t('unitPoints', language)}</div>
        </div>
        <div className="p-3 rounded transition-all hover:bg-opacity-50" style={{ background: 'rgba(240, 185, 11, 0.05)' }}>
        ...
        <div className="p-3 rounded transition-all hover:bg-opacity-50" style={{ background: 'rgba(240, 185, 11, 0.05)' }}>
          ...
          <div className="text-base font-bold mono" style={{ color: '#EAECEF' }}>
            {combinedData.length > MAX_DISPLAY_POINTS
              ? t('recentCount', language, { count: MAX_DISPLAY_POINTS })
              : t('allData', language)}
          </div>
        </div>
      </div>
