      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="rounded-2xl p-5 relative overflow-hidden group hover:scale-105 transition-transform" style={{
          ...
          <div className="relative">
            ...
            <div className="text-xs" style={{ color: '#6366F1' }}>📊 {t('trades', language)}</div>
          </div>
        </div>
        ...
        <div className="rounded-2xl p-5 relative overflow-hidden group hover:scale-105 transition-transform" style={{
          ...
          <div className="relative">
            ...
            <div className="text-xs" style={{ color: '#6EE7B7' }}>📈 {t('usdtAverage', language)}</div>
          </div>
        </div>
        ...
        <div className="rounded-2xl p-5 relative overflow-hidden group hover:scale-105 transition-transform" style={{
          ...
          <div className="relative">
            ...
            <div className="text-xs" style={{ color: '#FCA5A5' }}>📉 {t('usdtAverage', language)}</div>
          </div>
        </div>
      </div>
          ...
          <div className="relative">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <div className="text-xs" style={{ color: '#94A3B8' }}>{t('sharpeRatioSubtitle', language)}</div>
              </div>
            </div>
                ...
                <div className="text-right mb-2">
                  <div className="text-sm font-bold px-3 py-1 rounded-lg" style={{
                    ...
                    {performance.sharpe_ratio >= 2 ? t('sharpeExcellent', language) :
                     ...
                     performance.sharpe_ratio >= 0 ? t('sharpeVolatile', language) : t('sharpeNeedsAdjustment', language)}
                  </div>
                </div>
              )}
            </div>
            ...
            {performance.sharpe_ratio !== undefined && performance.sharpe_ratio !== null && (
              <div className="rounded-xl p-4" style={{
                ...
                <div className="text-sm leading-relaxed" style={{ color: '#DDD6FE' }}>
                  {performance.sharpe_ratio >= 2 && t('sharpeExcellentDesc', language)}
                  {performance.sharpe_ratio >= 1 && performance.sharpe_ratio < 2 && t('sharpeGoodDesc', language)}
                  {performance.sharpe_ratio >= 0 && performance.sharpe_ratio < 1 && t('sharpeVolatileDesc', language)}
                  {performance.sharpe_ratio < 0 && t('sharpeNeedsAdjustmentDesc', language)}
                </div>
              </div>
            )}
          </div>
          ...
          <div className="relative">
            ...
            <div className="rounded-xl p-4" style={{
              ...
              <div className="text-sm leading-relaxed" style={{ color: '#FEF3C7' }}>
                {(performance.profit_factor || 0) >= 2.0 && t('profitFactorExcellentDesc', language, { factor: (performance.profit_factor || 0).toFixed(1) })}
                {(performance.profit_factor || 0) >= 1.5 && (performance.profit_factor || 0) < 2.0 && t('profitFactorGoodDesc', language)}
                {(performance.profit_factor || 0) >= 1.0 && (performance.profit_factor || 0) < 1.5 && t('profitFactorFairDesc', language)}
                {(performance.profit_factor || 0) > 0 && (performance.profit_factor || 0) < 1.0 && t('profitFactorPoorDesc', language)}
              </div>
            </div>
          </div>
        </div>
              ...
              <table className="w-full">
                <thead className="sticky top-0 z-10">
                  <tr style={{ background: 'rgba(15, 23, 42, 0.95)', backdropFilter: 'blur(10px)' }}>
                    <th className="text-left px-4 py-3 text-xs font-semibold" style={{ color: '#94A3B8' }}>{t('symbol', language)}</th>
                    <th className="text-right px-4 py-3 text-xs font-semibold" style={{ color: '#94A3B8' }}>{t('trades', language)}</th>
                    <th className="text-right px-4 py-3 text-xs font-semibold" style={{ color: '#94A3B8' }}>{t('winRate', language)}</th>
                    <th className="text-right px-4 py-3 text-xs font-semibold" style={{ color: '#94A3B8' }}>{t('totalPnL', language)} (USDT)</th>
                    <th className="text-right px-4 py-3 text-xs font-semibold" style={{ color: '#94A3B8' }}>{t('avgPnL', language)}</th>
                  </tr>
                </thead>
                <tbody>
              ...
              performance.recent_trades.map((trade: TradeOutcome, idx: number) => {
                ...
                return (
                  <div key={idx} className="rounded-xl p-4 backdrop-blur-sm transition-all hover:scale-[1.02]" style={{
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-3 text-xs">
                      <div>
                        <div style={{ color: '#94A3B8' }}>{t('quantity', language)}</div>
                        <div className="font-mono font-semibold" style={{ color: '#CBD5E1' }}>
                          {trade.quantity ? trade.quantity.toFixed(4) : '-'}
                        </div>
                      </div>
                      <div className="text-right">
                        <div style={{ color: '#94A3B8' }}>{t('leverage', language)}</div>
                        <div className="font-mono font-semibold" style={{ color: '#FCD34D' }}>
                          {trade.leverage ? `${trade.leverage}x` : '-'}
                        </div>
                      </div>
                      <div>
                        <div style={{ color: '#94A3B8' }}>{t('positionValue', language)}</div>
                        <div className="font-mono font-semibold" style={{ color: '#CBD5E1' }}>
                          {trade.position_value ? `$${trade.position_value.toFixed(2)}` : '-'}
                        </div>
                      </div>
                      <div className="text-right">
                        <div style={{ color: '#94A3B8' }}>{t('marginUsed', language)}</div>
                    ...
                    <div className="flex items-center justify-between text-xs" style={{ color: '#94A3B8' }}>
                      <span>⏱️ {formatDuration(trade.duration, language, t)}</span>
                      {trade.was_stop_loss && (
...
// 格式化持仓时长
function formatDuration(duration: string | undefined, language: 'en' | 'zh', t: (key: string, lang: 'en' | 'zh') => string): string {
  if (!duration) return '-';
  ...
  const seconds = secondsStr ? parseFloat(secondsStr.replace('s', '')) : 0;

  let result = '';
  if (hours > 0) result += `${hours}${t('hour', language)}`;
  if (minutes > 0) result += `${minutes}${t('minute', language)}`;
  if (seconds > 0 && hours === 0) result += `${Math.round(seconds)}${t('second', language)}`;
